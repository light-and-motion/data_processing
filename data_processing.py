...
class Data_Processing: 
    ...
    def create_csv_dataframe(self, file, startLine): 
        ...
        startLine (float): Row number to begin processing CSV file. 
        """
    ...
    def process_data(self, wb, df, config_df, output_col_letters):
        ...
        self.adjust_column_widths(ws, df, output_col_letters, title_inputs, new_titles)
        return wb

   
    def adjust_column_widths(self, ws, mapping_df, output_col_letters, title_inputs, new_titles):
        ...
        for letters in output_col_letters:
            str_series = mapping_df[title_inputs.iloc[i]].astype(str)
    ...
    def read_in_values(self, ws, mapping_df, new_title, title_input, col_num):
        ...
        cellRow = 2 
        i = 0
        size = mapping_df[col_index].size
        while (i < size):   
            ws.cell(row = cellRow, column = col_num).value = mapping_df.loc[i,col_index]
            cellRow += 1
            i += 1
    ...
    def create_chart(self,wb, mapping_df, x_axis, y_axis, config_df_1, config_df_2): 
        ...
        row_size = mapping_df[title_inputs.loc[0]].size
    ...
    def make_jpeg(self, mapping_df, x_axis_row, y_axis_row, config_df_1, config_df_2, output_name, jpeg_choice, pdf_choice):  
        ...
        x_axis = mapping_df[title_inputs[x_axis_row.index[0]]].dropna() 
    ...
    def make_pdf(self, output_name,  mapping_data_df, create_chart): 
        ...
        if (not create_chart): 
            ...
            pdfkit.from_string(mapping_data_df.to_html(), df_file, configuration = config)
            paths = [df_file, os.getcwd() + '\\' + output_name + '_chart.pdf' ]
